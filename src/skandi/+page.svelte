<script lang="ts">
  ...
  async function sendDataWithForm(event: any) {
    ...
    if (first === second || first === third || first === fourth || first === fifth || first === sixth || first === seventh ||
        second === third || second === fourth || second === fifth || second === sixth || second === seventh ||
        third === fourth || third === fifth || third === sixth || third === seventh ||
        fourth === fifth || fourth === sixth || fourth === seventh ||
        fifth === sixth || fifth === seventh ||
        sixth === seventh) {
    ...
    seventh = arr[6]
      ...
      const formData: RequestPayload = {
        ...
        seventh
			...
			const response = await fetch('https://lottery-gamma-three.vercel.app/skandi', {
				method: 'POST',
				headers: {
    ...
    <div>
      ...
      <div class="pt-10 flex flex-col items-center justify-center font-poppins text-3xl font-semibold">
        <form on:submit={sendDataWithForm}>
          ...
          <input class="rounded bg-slate-500 opacity-90 border border-gray-400 text-white font-bold text-opacity-100" type="number" bind:value={seventh} min={minValue} max={maxValue} required>
          <button
        ...
        {#if show === true}
          <div class="flex flex-col items-center justify-center">
            {#each lotteryData as lot}
              <p><span class="text-2xl font-normal">Amount:</span> {formats(lot.sc_total)} <span class="text-2xl font-normal"> Ft</span></p>
              <p><span class="text-2xl font-normal">Year/Week:</span> {lot.sc_year}/{lot.sc_week}</p>
            {/each}
          </div>
        {/if}
      </div>
