  ...
  <div class="main w-full h-full bg-no-repeat bg-cover" style="background-image: url('balls.png');">
    ...
    <div class="h-screen flex items-center justify-center font-poppins text-3xl font-semibold">
      <form on:submit={sendDataWithForm}>
        ...
        <input class="rounded bg-slate-500 opacity-90 text-white" type="number" bind:value={second} min={minValue} max={maxValue} required>
        <input class="rounded bg-slate-500 opacity-90 text-white" type="number" bind:value={third} min={minValue} max={maxValue} required>
        <input class="rounded bg-slate-500 opacity-90 text-white" type="number" bind:value={fourth} min={minValue} max={maxValue} required>
        <input class="rounded bg-slate-500 opacity-90 text-white" type="number" bind:value={fifth} min={minValue} max={maxValue} required>
        <button
          class="text-white"
          id="btn"
          type="submit"
